...
type OptionsButtonProps = React.ButtonHTMLAttributes<HTMLButtonElement> & {
  options: OptionsButtonOption[] | Readonly<OptionsButtonOption[]>;
          ...
          <div className="relative">
            <div className="inline-flex divide-x divide-black rounded-lg shadow-sm">
              <div className="inline-flex divide-x divide-neutral-600 rounded-lg shadow-sm">
                <Button type="button" onClick={onClick} className="inline-flex items-center rounded-l-lg rounded-r-none border border-transparent bg-black py-2 pl-3 pr-4 text-white shadow-sm" {...props}>
            ...
            <Transition
              ...
              leaveTo="opacity-0"
            >
              <Listbox.Options className="absolute bottom-8 right-0 z-50 mb-2 w-72 origin-top-right divide-y divide-gray-200 overflow-hidden rounded-lg bg-white shadow-lg ring-1 ring-black/5 focus:outline-none">
                {options.map((option) => (
                  <Listbox.Option
                    key={option.label}
                    className={({ active }) =>
